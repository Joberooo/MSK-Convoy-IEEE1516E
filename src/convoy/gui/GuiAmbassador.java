...
public class GuiAmbassador extends AbstractAmbassador {
    protected final GuiFederate federate;

    public static double federateTime        = 0.0;
    public static double federateLookahead   = 1.0;

    protected GuiAmbassador(GuiFederate federate) {
    ...
    protected void log(String message) {
        System.out.println( GuiFederate.FEDERATE_NAME + "Ambassador: " + message );
    }
    ...
    public void reflectAttributeValues(ObjectInstanceHandle theObject, AttributeHandleValueMap theAttributes, byte[] tag, OrderType sentOrdering, TransportationTypeHandle theTransport, LogicalTime time, OrderType receivedOrdering, SupplementalReflectInfo reflectInfo) {
        ...
        GuiFederate.SingleRouteSection singleRoute = new GuiFederate.SingleRouteSection();

        boolean isItVehicle = false;
        GuiFederate.SingleVehicle singleVehicle = new GuiFederate.SingleVehicle();
        ...
        boolean add = true;
        if(isItRoute){
            for(GuiFederate.SingleRouteSection s : federate.singleRouteSectionList){
        ...
        if(isItVehicle){
            for(GuiFederate.SingleVehicle v : federate.singleVehiclesList){
